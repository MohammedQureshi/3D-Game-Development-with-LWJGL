...
public class Loader {
	...
	public int loadGuiTexture(String fileName) {
		Texture texture = null;
		try {
			texture = TextureLoader.getTexture("PNG",Class.class.getResourceAsStream("/res/gui/"+fileName+".png"));
			GL30.glGenerateMipmap(GL11.GL_TEXTURE_2D);
			GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MIN_FILTER, GL11.GL_LINEAR_MIPMAP_LINEAR);
	...
	public void deleteTextData(Integer vao, Integer vbo){//it's important to use Integer here and not int
		vaos.remove(vao);
		vbos.remove(vbo);
